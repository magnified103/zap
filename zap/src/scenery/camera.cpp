...
void camera::render(sdl::renderer_handle renderer, player &player, map_grid &grid, float width,
                    float height) {
    int ray_count = 100;
    ...
    for (int i = 0; i < ray_count; i++) {
        ...
        auto hit = grid.hit_distance(player_position, {player_position, player_position + delta});
        hits.emplace_back(hit.first * std::cos(base_angle - horizontal_angle), hit.second);
...
void camera::render_hits(sdl::renderer_handle renderer,
                         std::vector<std::pair<float, sdl::color>> hits, float delta, float horizon,
                         float base_height, float base_color) {
    float x = 0;
    for (auto hit : hits) {
        ...
        renderer.fill_rect(sdl::frect{x, horizon - wall_height / 2, delta, wall_height});
        x += delta;
    }
}

void camera::render3d(sdl::renderer_handle renderer, player &player, map_grid &grid, float width,
                      float height) {
    ...
    // center
    vec3 center = eye_3d + vec3{std::cos(horizontal_angle), std::tan(vertical_angle),
    ...
    std::sort(walls.begin(), walls.end(), [&eye_2d](const wall &a, const wall &b) {
    ...
    for (const auto &wall : walls) {
        ...
        const auto render_no_check = [&](const vec4 &vec_a, const vec4 &vec_b, const vec4 &vec_c) {
            const SDL_Vertex vertex_a{{width * (vec_a.x + 1), height * (-vec_a.y + 1)},
                                      wall.color.get()};
            const SDL_Vertex vertex_b{{width * (vec_b.x + 1), height * (-vec_b.y + 1)},
                                      wall.color.get()};
            const SDL_Vertex vertex_c{{width * (vec_c.x + 1), height * (-vec_c.y + 1)},
                                      wall.color.get()};
            renderer.render_triangles({vertex_a, vertex_b, vertex_c});
        };

        // render a normalized triangle with additional checks
        const auto render = [&](const vec4 &vec_a, const vec4 &vec_b, const vec4 &vec_c) {
            const auto validate = [](const vec4 &a) -> bool {
                return std::abs(a.x) <= 1.0f && std::abs(a.y) <= 1.0f && std::abs(a.z) <= 1.0f;
            };
        ...
        if (!debug) {
            sdl::log_info("endpoints: (%.2f, %.2f), (%.2f, %.2f)", wall.endpoints[0].x,
                          wall.endpoints[0].y, wall.endpoints[1].x, wall.endpoints[1].y);
            sdl::log_info("translated: (%.2f, %.2f, %.2f, %.2f), (%.2f, %.2f, %.2f, %.2f)", a.x,
                          a.y, a.z, a.w, b.x, b.y, b.z, b.w);
        }
    }
    debug = true;
}