...
void monitor::render(const std::string &text, const sdl::font &font, const float &height_offset,
    ...
    const float scaled_width = (icon_width + per_letter_width * text.size()) / screen_width * 2.0f;
    ...
    render_2d_rectangle(vec2{-1.0f + scaled_icon_width, -1.0f + scaled_height_offset},
...
void hotbar::render(const inventory &inventory, const std::vector<GLuint> &texture_ids,
...
void hud::render_scene(int hp, int ammo, const inventory &inventory,
                       const std::vector<GLuint> &texture_ids, const float &screen_width,
                       const float &screen_height, const GLuint &shader_program) {
    if (!font) {
...
void hud::render_end_screen(const std::vector<GLuint> &texture_ids, const float &screen_width,
                            const float &screen_height, const GLuint shader_program) {
