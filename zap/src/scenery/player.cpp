...
point2d<float> player::try_move(camera &camera, int move_type, float t) const {
    ...
    return p + point2d<float>{std::cos(angle), std::sin(angle)} * movement_scalar[move_type] *
                   (t * horizontal_speed);
}
...
void player::render(sdl::renderer_handle renderer, sdl::texture_handle dot_texture) const {
