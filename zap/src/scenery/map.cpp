...
sdl::surface map_grid::get_surface() const {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            if (grid[i][j] == 1) {
                surface.fill_rect(sdl::rect{j * grid_size, i * grid_size, grid_size, grid_size},
...
std::optional<std::pair<point2d<float>, sdl::color>> map_grid::ray_cast(ray<float> ray) const {
    ...
    point2d<float> d{static_cast<float>(width * grid_size), static_cast<float>(height * grid_size)};
    auto intersection = get_intersection(ray, {a, b});
    if (intersection) {
        float distance = (*intersection - ray.a).abs();
        if (!best_intersection || minimum_distance > distance) {
...
std::pair<float, sdl::color> map_grid::hit_distance(point2d<float> source, ray<float> ray) const {
...
std::vector<wall> map_grid::get_walls() const {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            if (grid[i][j] == 1) {
                walls.push_back(
                    {{{j * cell_size, i * cell_size}, {(j + 1) * cell_size, i * cell_size}},
                     color_parallel_x});
                walls.push_back(
    ...
    walls.push_back(
        {{{width * cell_size, 0}, {width * cell_size, height * cell_size}}, color_parallel_x});
    walls.push_back(
        {{{0, height * cell_size}, {width * cell_size, height * cell_size}}, color_parallel_z});
    return walls;
}